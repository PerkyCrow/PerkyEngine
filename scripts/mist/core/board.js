...
export default class Board {
    ...
    setReagent (reagent, order = this.order++) {
        ...
        this.grid.setCell(reagent.x, reagent.y, reagent)
        reagent.order = order
    ...
    syncReagents () {
        this.grid.forEachDefinedCellWithCoords((reagent, x, y) => {
            reagent.x = x
            reagent.y = y
        })
    }
    ...
    toArray () {
        const reagents = []
        this.grid.forEachDefinedCell(reagent => reagents.push(reagent))
    ...
    debug () {
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                const reagent = this.getReagent({x, y})
                col.push(reagent ? reagent.name.slice(0, 2) + reagent.name.slice(-1) : ' - ')
            }
        }
