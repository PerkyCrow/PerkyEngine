...
describe(Lab, () => {
    ...
    beforeEach(() => {
        game = {
            random: random.createNew('hello'),
            weightedChoice: (choices) => game.random.weightedChoice(choices)
        }
    })
    ...
    test('buildReagent', () => {
        ...
        expect(lab.buildReagent({}, game)).toEqual({x: 0, y: 0, name: 'fire'})
        expect(lab.buildReagent({name: 'water'})).toEqual({x: 0, y: 0, name: 'water'})
        expect(lab.buildReagent({x: 1, y: 1}, game)).toEqual({x: 1, y: 1, name: 'match'})
    ...
    test('buildCluster', () => {
        const lab = createLab({unlockedCount: 4, activeCount: 4})
        {
            ...
            expect(cluster.reagents.map(el => el.name)).toEqual(['fire', 'match'])
        }
        {
            const cluster = lab.buildCluster({}, game)
            expect(cluster.reagents.map(el => el.name)).toEqual(['fireBranch', 'fireBranch'])
        }
        {
            const cluster = lab.buildCluster({pairs: true}, game)
            expect(cluster.reagents.map(el => el.name)).toEqual(['branch', 'branch'])
        }
        {
            const cluster = lab.buildCluster({count: 1}, game)
            expect(cluster.reagents.map(el => el.name)).toEqual(['fireBranch'])
        }
    })
